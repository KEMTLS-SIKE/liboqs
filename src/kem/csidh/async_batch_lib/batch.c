...
static int crypto_kem_async_batch_keypair(unsigned char *pk,
  ...
  for (i = 0; i < n; i++) {
    int ret;
    ret = OQS_KEM_csidh_p512_keypair(pk + i * sizeof(public_key),
                                           sk + i * sizeof(private_key));
    if (ret != 0) {
      return 1;
    }
  }
...
{    
    if (crypto_kem_async_batch_keypair(store->pks, store->sks, batch_size) == 0) { // success
...
BATCH_STORE *BATCH_STORE_new(size_t batch_size)
{
    ...
    pks_len = batch_size * sizeof(public_key);
    sks_len = batch_size * sizeof(private_key);
...
int BATCH_STORE_get_keypair(BATCH_STORE *store, KEM_KEYPAIR *kp)
{
    ...
    pk_bytes = sizeof(public_key);
    sk_bytes = sizeof(private_key);
...
int csidh_p512_async_batch_init(void)
{
    BATCH_CTX *ctx = NULL;
 ...
 */
int csidh_p512_async_batch_deinit(void)
{
    if (pthread_mutex_lock(&global_mut) != 0) {
        return 1;
    }
