...
OQS_KEM *OQS_KEM_csidh_p512_new(void) {
	...
	kem->init = csidh_p512_async_batch_init;
	kem->deinit = csidh_p512_async_batch_deinit;
	kem->keypair_async = OQS_KEM_csidh_p512_async_keypair;
	...
	kem->encaps_ciphertext = OQS_KEM_csidh_p512_encaps_ciphertext;
...
OQS_API OQS_STATUS OQS_KEM_csidh_p512_async_init(void) {
	return csidh_p512_async_batch_init();
}

OQS_API OQS_STATUS OQS_KEM_csidh_p512_async_deinit(void) {
	return csidh_p512_async_batch_deinit();
}

int OQS_KEM_csidh_p512_async_keypair(unsigned char *pk, unsigned char *sk)
{
	KEM_KEYPAIR kp;
  	kp.pk = pk;
  	kp.sk = sk;
...
OQS_API OQS_STATUS OQS_KEM_csidh_p512_encaps_ciphertext(uint8_t *ciphertext, uint8_t *ephemeral_secret, const uint8_t *pkey) {
	(void) pkey; // unused argument

	return OQS_KEM_csidh_p512_async_keypair(ciphertext, ephemeral_secret);
}
