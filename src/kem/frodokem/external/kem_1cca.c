...
{ // FrodoKEM's key encapsulation
    ...
    uint8_t mu[BYTES_MU];                                     // contains secret data
    ...
    // generate random mu, compute (seedSE || k) = G_2(mu)
    randombytes(mu, BYTES_MU);
    shake(G2out, CRYPTO_BYTES + CRYPTO_BYTES, mu, BYTES_MU);
...
{ // FrodoKEM's key decapsulation
    ...
    // Generate (seedSE' || k') = G_2(mu')
    shake(G2out, CRYPTO_BYTES + CRYPTO_BYTES, muprime, BYTES_MU);

    // Prepare input to F
    memcpy(Fin_ct, ct, CRYPTO_CIPHERTEXTBYTES);

    memcpy((uint8_t*)Fin_k, (uint8_t*)kprime, CRYPTO_BYTES);
    shake(ss, CRYPTO_BYTES, Fin, CRYPTO_CIPHERTEXTBYTES + CRYPTO_BYTES);
